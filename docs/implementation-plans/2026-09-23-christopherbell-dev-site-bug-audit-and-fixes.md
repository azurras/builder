...
- Tests and evidence: Add a Pester regression for a candidate executable that writes a safe startup diagnostic and exits before binding; assert the surfaced failure includes the bounded diagnostic and exit code, without exposing configured environment values. Run focused `Production.Deploy.Tests.ps1` and full `:website:check`; after merge, let the SYSTEM poller build and exercise the new SHA.
- Verification: Not started. The next required checkpoint is a reviewed, validated plan followed by implementation and Pester evidence. The latest confirmed production listener is PID 14424 and no candidate listener is present.
