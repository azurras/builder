...
- Symbols: `productionAuthorityRootIsFixedAndSelfMintedOwnerOnlyFilesAreRejected`, `UserPrincipalLookupService.lookupPrincipalByName`, and the Windows SYSTEM account name.
- Inspection: After the user approved elevation, read the protected auto-deploy state and confirmed `ChristopherBellAutoDeploy` had been disabled since 2026-09-05. The supported `prod.cmd auto-install -WhatIf` preflight passed; running documented `prod.cmd auto-install` from the worktree whose Git tree matches merged `origin/main` restored the SYSTEM poller. It detected remote SHA `5f89e5c286e9c96b1d45dcfbd88bc036cc39c34c`, built 2,282 tests, and failed one after 6m45s. The Gradle daemon report names `FinalizeEvidenceLoaderTest.productionAuthorityRootIsFixedAndSelfMintedOwnerOnlyFilesAreRejected` with `UserPrincipalNotFoundException` at the lookup line: Windows exposes the SYSTEM process name as `SYSTEM`, which is not its canonical account name `NT AUTHORITY\\SYSTEM`. The failed release worktree was correctly cleaned up; the live service was never stopped. The auto-deploy state now records this SHA as failed, so same-SHA attempts respect configured backoff while a newer merged SHA is immediately eligible.
- Required skills: `write-jane-street-style-code` and focused regression verification.
- Behavior: Resolve the SYSTEM account to its qualified Windows principal name before assigning the temporary owner in this test, while preserving behavior for other Windows accounts and preserving the test's self-minted-owner rejection assertion.
...
- Tests and evidence: The actual SYSTEM deployment build provides the red case: 2,282 tests, one failure, 300 skipped. Focused `FinalizeEvidenceLoaderTest` passed all eight tests, including canonical SYSTEM principal mapping and unchanged lookup behavior for other accounts. Full native Windows `:website:check` passed in 5m27s, including Windows PowerShell suites (183 installer tests discovered; 75 shared-folder worker tests passed) and the Java suite. PR #1400 is open at `be531807ad0846928306e65c6c07980b205430f0`; required Ubuntu, macOS, Windows, Java/JavaScript CodeQL and dependency-review checks are queued. After merge, let the enabled SYSTEM poller build the new SHA and record release SHA, listener rotation, readiness/routes, and both freshness APIs.
- Verification: In progress. At 2026-09-23 19:16 CDT the poller had failed merged SHA `5f89e5c286e9c96b1d45dcfbd88bc036cc39c34c` on the single SYSTEM principal lookup test; at 19:17 its task was enabled and Ready, with existing production service still Running on port 8080/PID 14424. The user approved the elevation prompt. The new fixture mapping and full local check now pass, but required PR checks and production deployment/health/freshness acceptance remain pending.
