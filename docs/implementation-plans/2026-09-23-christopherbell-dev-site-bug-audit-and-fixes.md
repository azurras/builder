...
- Verification: PR #1402's protected-tool refresh succeeded through `prod.cmd auto-install`, and its installed module hash matched the merged source. Once the 900-second failure backoff expired, the updated poller attempt at 2026-09-24 02:26:01 UTC stopped before candidate startup because the shared `logs` directory does not satisfy `Assert-ProtectedProductionPath` (`Protected production path must disable ACL inheritance`). No shared ACL was changed. PR #1403 (`8f2c4e10`) adds a dedicated `logs\candidate` child with the exact protected SYSTEM/Administrators ACL; focused Pester tests passed 90/90, Windows PowerShell 5.1 module import passed, and native Windows `:website:check` passed in 3m29s. PR CI and production retry are pending. Production remains Running on PID 14424 with readiness HTTP 200; WFL freshness remains `2026-08-02T22:44:50.963Z` and Cane's history remains nine weeks through 2026-09-07. Production acceptance remains in progress until the updated bundle retry and post-deployment data recovery are verified.
...
- Tests and evidence: Add failing Pester cases for status path safety, exact read-only ACL, standard-user-readable status without config access, stale/malformed status handling, sanitized error output, and nonzero surfaced deployment failure. Run focused AutoDeploy Pester and native `:website:check`; validate README contract and module import under Windows PowerShell 5.1.
- Verification: From a standard-user session, run `prod.cmd auto-status` and confirm it returns sanitized status without prompting or opening protected paths. Where deployment remains unavailable without UAC, record public readiness/data freshness and process/listener evidence without claiming production acceptance.
...
- Files: `ops/production/windows/modules/Production.AutoDeploy.psm1`, `ops/production/windows/modules/Production.Deploy.psm1`, `ops/production/windows/tests/Production.AutoDeploy.Tests.ps1`, and `README.md`.
...
- Required skill: `write-jane-street-style-code` before code changes.
- Behavior: Once the existing SYSTEM task is installed, routine merges that change deployer tools require no administrator prompt. On a trusted main-tree change, the task stages an immutable, versioned protected tool bundle from that exact commit, verifies the tree identity and all files, registers the new task action, releases `deploy.lock`, and lets the next invocation run the new version. It does not deploy from a dirty developer checkout or overwrite the currently executing bundle.
...
- Effects and failures: The task may add a version directory and replace its own scheduled action while holding the production lock. It does not stop or mutate its executing files. Registration happens only after complete publication; the lock is released before any explicit start. Failure keeps the current action and reports safe retry status.
- Tests and evidence: Add Pester tests for exact-commit tool source, tree-hash match, immutable staging, partial-stage cleanup, ACL/reparse rejection, old-action preservation on failure, and no start before lock release. Run focused Pester and native `:website:check`; import the resulting module under Windows PowerShell 5.1.
- Verification: Confirm `auto-status` remains readable by a standard user across a tool-version switch. Record process/listener/readiness and public data evidence. Production acceptance still requires the deployed release SHA and recovered data freshness; do not use elevated inspection in this workstream.
