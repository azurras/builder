...
- Tests and evidence: The actual SYSTEM deployment build provides the red case: 2,282 tests, one failure, 300 skipped. Add focused coverage for the canonical SYSTEM principal name, run `FinalizeEvidenceLoaderTest`, full `:website:check`, and the packaged candidate checks. After merge, let the enabled SYSTEM poller build the new SHA and record release SHA, listener rotation, readiness/routes, and both freshness APIs.
- Verification: In progress. At 2026-09-23 19:16 CDT the poller had failed merged SHA `5f89e5c286e9c96b1d45dcfbd88bc036cc39c34c` on the single SYSTEM principal lookup test; at 19:17 its task was enabled and Ready, with existing production service still Running on port 8080/PID 14424. The user approved the elevation prompt. Do not mark deployment accepted until the next SHA is active and production health/data freshness are read back.
