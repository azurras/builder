...
- Verification: Confirmed monthly schedule and catch-up comparison in source. Startup catch-up fails to retry after the current month's scheduled occurrence when the prior successful completion predates that occurrence; add a deterministic regression test in Task 2.
...
- Verification: The failing test must reproduce the missed catch-up before implementation and pass after the fix. Confirm the full native gate, clean diff, isolated test database, candidate readiness and representative HTTP routes.
...
## Completion Criteria
The new regression test fails on baseline and passes after the fix; native checks and alternate-port runtime proof pass; a reviewed PR passes required CI; supported deployment and exact production acceptance evidence are recorded. Complete issue handling if a source issue exists, plus dated Builder session records/index validation/publication. Report remaining stale-data cause or verification limitations explicitly.
