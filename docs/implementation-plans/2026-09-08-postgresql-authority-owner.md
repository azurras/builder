...
- Behavior: preserve exit code 2 and stdout evidence while forwarding bounded, allowlisted exception class names for both snapshot and migration CLI invocations. For SQL exceptions only, also emit the five-character SQLSTATE so PostgreSQL failure category is actionable.
...
- Boundary/API: an opt-in safe-diagnostic switch on the checked process helper; enabled only for the two exact migration CLI main classes. Filter exception class lines and five-character SQLSTATE lines only. Update existing contract assertions as needed, without adding tests.
...
## Code Changes
Separate explicit trusted ownership from ordinary user ownership, reusing the established trusted write-principal set. Do not change ACL creation or relax cryptographic evidence verification. Preserve and forward only allowlisted exception class metadata and SQLSTATE when either migration Java CLI fails; do not print exception messages or sensitive data. A failed finalize attempt is known to be caused by `MigrationStorageException` / `PSQLException`, but its SQLSTATE is not captured; the next guarded cutover will identify the PostgreSQL failure category.
