...
## Newer-Release Progress After Recovery Failure (2026-09-24 21:50 UTC)

Source review of merged PR #1437 found that `Invoke-AutoDeployOnce` propagated active-service recovery failure (or returned during recovery backoff) before querying remote `main`. That prevented a newer release from reaching the existing guarded deploy path while the currently active release was unhealthy. On branch `codex/deploy-unhealthy-release-forward-20260924` from merged main `3b11df8a10846adf5a686f1917254ec7583e3e2f`, a regression test first failed because remote lookup/deployment were skipped after recovery threw. The implementation now continues to query remote `main`, deploys only a newer SHA through normal orchestration/candidate verification, and retains same-SHA recovery failure/backoff. A second regression verifies recovery backoff does not block a newer SHA; another verifies remote-check errors remain `REMOTE_CHECK` and preserve protected recovery retry state.

Automatic-deploy, deploy, operations, and command Pester suites passed 288/288 on PowerShell 7.6.6/Pester 5.9 and 288/288 on Windows PowerShell 5.1/Pester 5.9. PowerShell parser checks and `git diff --check` passed. This is local source verification; this follow-up has not yet merged or reached the production poller. At the latest live check (2026-09-24 21:35 UTC), the service remained stopped with SCM exit code 1067 and public readiness returned HTTP 502. Production recovery and runtime acceptance remain incomplete.
