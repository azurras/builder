...
## 2026-09-24 12:53 - Remove PostgreSQL support and validate MongoDB-only build

Requested removal of PostgreSQL support after the user confirmed the project is no longer switching to PostgreSQL. Continued from the published MongoDB-only implementation plan (`docs/implementation-plans/2026-09-24-christopherbell-dev-mongodb-only-persistence.md`, Builder commit `05d1804`). Implemented in spoke branch `codex/mongodb-only-persistence-20260924` at base `f4382d118db9db43efc952dc08e96649e2c70753`, local source commit `06b1c6f6`.

Removed the PostgreSQL persistence adapters, relational migration/cutover tooling, PostgreSQL ops modules and commands, configuration selectors, SQL/Flyway/JPA/PostgreSQL dependencies, stale verification-metadata components, local PostgreSQL Compose service, and active cutover documentation. MongoDB adapters are now unconditional. Production env/service examples require MongoDB; installer refresh removes retired PostgreSQL fields from existing deployment JSON. Removed the orphaned JPA browser-session classes after the first compile exposed their dependency on the deleted persistence stack. Fixed a Pester regex assertion to correctly inspect the whole multiline env template. Historical dated plans, reports, and session records were preserved. Remaining PostgreSQL text is limited to upgrade cleanup of legacy deployment keys and assertions that retired settings/commands are absent.

Validation: `:website:compileJava` and `:website:compileTestJava` passed. Full `:website:check` passed in 5m 2s using isolated Gradle home, database `test`, and explicit disposable Mongo URI `127.0.0.1:27019`; JUnit XML shows 1,960 tests, 0 failures/errors, 108 skipped; JavaScript 343 passed; deployment Pester 201 passed/1 skipped; worker Pester 75 passed. Mongo integration opt-ins were unset. The first broad check was interrupted before its Java tests because the environment has a live MongoDB listener on 27017; reran with explicit isolated URI. No production Mongo listener, data, service, configuration, or port 8080 was changed.

Attempted Spring Boot test-profile startup on port 18081 against the disposable MongoDB. Spring connected to 27019, then the app failed closed in migration `015-require-domain-collection-schema` because a fresh disposable database lacks the exact active cutover ledger. Did not fabricate or copy ledger evidence. Candidate exited before readiness; disposable Mongo process was stopped. Temporary database files remain under `%TEMP%\codex-mongodb-only-89e30307754a4adbb1ca537a4ae4105e` because the command policy rejected recursive cleanup. Full runtime report: `docs/test-reports/2026-09-24-christopherbell-dev-mongodb-only-runtime-verification.md` (blocked). Runtime acceptance and production deployment remain outstanding pending an authorized restored Mongo backup carrying the valid cutover ledger.

## 2026-09-24 13:18 Central Daylight Time - MongoDB-only PR CI passed

Opened draft PR #1432 for removal of PostgreSQL support: https://github.com/azurras/christopherbell.dev/pull/1432. CI first caught a test-isolation bug: the health endpoint fixture used stub database probes, while Spring Boot 4 also auto-enabled the MongoDB health indicator and returned HTTP 503 in CI without a Mongo server. Corrected the fixture property to `management.health.mongodb.enabled=false` after confirming the Spring Boot 4 property name. Final commit `3935d3d735310264ee5e2b7e3eda11b801bdcee7` passed Linux, macOS, and Windows builds, all CodeQL language analyses, and dependency review. A local targeted Gradle rerun was unable to start because the Gradle launcher could not establish a loopback connection; the full CI matrix verified the test change. Runtime readiness remains unverified because migration 015 requires an authentic active cutover ledger absent from an empty disposable database. PR remains draft; no production change, merge, or deployment occurred. Next user input needed only to provide or identify an authorized Mongo backup containing that ledger so it can be restored to a disposable database for runtime proof.


## 2026-09-24 13:46 Central Daylight Time - automatic deploy poller status visibility

## 2026-09-24 13:45 Central Daylight Time - Add poller registration to automatic-deploy status

Implemented Task 34 from `docs/implementation-plans/2026-09-23-christopherbell-dev-site-bug-audit-and-fixes.md` in isolated worktree `A:\Projects\christopherbell.dev-worktrees\auto-status-poller-state-20260924`, branch `codex/auto-status-poller-state-20260924`, commit `6fa17e6c722c9eb040f05bd7cb2c7c9f5d5115ce`, based on refreshed `origin/main` `f4382d118db9db43efc952dc08e96649e2c70753`. The authoritative spoke checkout remained untouched with its prior `gradlew.bat` modification and untracked directory preserved.

The read-only `auto-status` projection now reports poller registration/state separately from the sanitized status record: `NOT_REGISTERED`, `READY`, `RUNNING`, `DISABLED`, or `QUEUED`; query failures produce `UNKNOWN` with static reason codes and do not hide a readable status. It does not expose task action/principal, protected paths, or exception details. Operator guidance and regressions cover missing task with fresh `BACKING_OFF`, registered states, denied scheduler query, and unchanged unavailable-record reasons.

The new missing-task regression failed before implementation because `pollerState` was absent. PowerShell 7/Pester 5.9 and Windows PowerShell 5.1/Pester 5.9 each passed the automatic-deploy, operations, and command suites: 178 passed, 0 failed. `git diff --check` passed. A final read-only CLI run exited 0 and returned `available=True`, `freshness=FRESH`, `status=BACKING_OFF`, `pollerState=NOT_REGISTERED`, `pollerReason=TASK_NOT_REGISTERED`; this confirms the host's root automatic-deploy task is absent while a fresh failed-revision backoff record exists. No task, protected configuration, service, listener, or production deployment was changed.

This work is a console-only status projection, not an HTTP application change, so application startup reporting does not apply; the actual CLI runtime was exercised and recorded in Task 34. The generic runtime-report helper rejected a web-app report because there is no HTTP listener or endpoint interaction to report; no misleading report was created. Hosted CI, pull request publication, and any production update remain pending.

The separate MongoDB-only PR #1432 remains draft with green hosted checks; runtime startup proof still needs an authorized restored Mongo backup containing the active migration-015 ledger. The missing root poller observed above is a separate operational issue and does not by itself establish the cause of the earlier failed release attempt.


## 2026-09-24 13:56 Central Daylight Time - Harden uppercase HTML-tag regression coverage

While CI for the automatic-deploy status PR #1433 ran, reviewed the open JavaScript CodeQL alerts from refreshed `origin/main`. The three production-code `js/xss-through-dom` locations were not confirmed as vulnerabilities: notification targets are generated from fixed local route prefixes and encoded identifiers; mention links insert user text with `textContent`; free-text links are extracted only by an `http(s)` URL matcher and inserted with `textContent`. The two `js/bad-tag-filter` findings were valid gaps in the tests: the assertions used case-sensitive lower-case tag checks and a Node probe confirmed the pattern missed `<SCRIPT>`.

On isolated worktree `A:\Projects\christopherbell.dev-worktrees\uppercase-html-test-coverage-20260924`, branch `codex/uppercase-html-test-coverage-20260924` from `origin/main` `f4382d118db9db43efc952dc08e96649e2c70753`, updated the shared-folder and composer-preview tests to include uppercase script/image payloads and case-insensitive raw-tag assertions, retaining lower-case cases. Targeted Node built-in tests passed 14/14; `git diff --check` passed. `:website:jsTest` could not launch because Gradle failed with `java.io.IOException: Unable to establish loopback connection`; Node executed the affected test files directly. No production rendering code changed. This test-only follow-up has not yet been committed or published.
...
## 2026-09-24 14:47 Central Daylight Time - Correct auto-status and Mongo runtime proof

Corrected the prior production auto-poller diagnosis. I had incorrectly called the task missing because `Get-ScheduledTask` enumeration filtered the inaccessible SYSTEM task. Read-only `schtasks /Query` returned access denied, exact Task Scheduler COM lookup returned HRESULT `0x80070005`, and the status `toolRefreshAt` was current. Added exact root-task COM querying so access denied reports `UNKNOWN/ACCESS_DENIED`, confirmed file-not-found reports `NOT_REGISTERED/TASK_NOT_REGISTERED`, and other failures remain `UNKNOWN/QUERY_FAILED`. Added sanitized state mapping and regressions. PowerShell 7/Pester 5.9 and Windows PowerShell 5.1/Pester 5.9 each passed AutoDeploy, Operations, and Command suites: 180 passed, 0 failed. Live read-only CLI retained fresh `BACKING_OFF` status and reported `UNKNOWN/ACCESS_DENIED`. Commit `1af3969727337f6c719be2a5e955a0d42ab3688d` is in PR #1435; hosted Linux/macOS builds, CodeQL analyses, and dependency review passed, with Windows build still pending at recording time.

Found a current checksum-matching Mongo backup at `A:\Projects\christopherbell.dev-backups\christopherbell-native-20260924T193158Z.archive.gz`. Restored 78,239 docs with zero restore failures into disposable localhost MongoDB port 27019. MongoDB-only commit `3935d3d735310264ee5e2b7e3eda11b801bdcee7` started under test profile on port 18081 after overriding the target URI and disabling scheduled external integrations. Readiness was UP and root returned HTTP 200. Detailed evidence is in `docs/test-reports/2026-09-24-mongodb-only-restored-backup-runtime-verification.md`; the earlier empty-database failure report is preserved separately.

The first exploratory test-profile start omitted the monthly WFL-import override and initiated an Overpass startup catch-up request, which received HTTP 504. It only used the restored disposable database. Repeating with `--wfl.restaurant-import.monthly.enabled=false` reached readiness without that call. Added Task 37 to the site bug-audit plan to disable networked monthly import/catch-up only in the test profile and cover its regression.

Stopped candidate PID 36252 and disposable Mongo PID 24624; ports 18081 and 27019 are closed and production Mongo PID 5388 remains present. Host command review rejected recursive deletion of the disposable restore directory, which remains under `%TEMP%\codex-mongodb-only-restored-20260924T193158Z`; did not bypass the review. No production database, task, service, deployment, or application state was changed. No user input is currently needed; proceed autonomously with PR gates and the planned test-profile isolation fix.

## 2026-09-24 15:25 Central Daylight Time - Test-profile collector isolation runtime proof

Resumed Task 37 on `codex/test-profile-wfl-catchup-20260924`, based on merged `origin/main` `4b552a63a08c9333bdaa0d7827b23eb811920f37`. The full `:website:check` passed after disabling WFL monthly import in `application-test.yml`: 21 actionable Gradle tasks, 14 executed and 7 up-to-date; JavaScript 343/343 and Windows production Pester 75/75 passed. Task 37 was expanded in the published plan after runtime evidence showed a second test-profile startup side effect: Cane's collected 50/50 prices from its public API and wrote a snapshot to the disposable DB. Existing `canes-box-tracker.enabled` checks suppress both scheduled and startup collection. Added a profile-binding regression; it failed before setting `canes-box-tracker.enabled: false` in `application-test.yml`. Final focused Cane's configuration/service and WFL configuration/workflow suites passed 37/37. Final native `:website:check` passed in 5m06s with 21 actionable tasks (13 executed, 8 up-to-date); Windows production Pester passed 75/75. The unrelated `gradlew.bat` line-ending conversion remains unstaged and untouched.

A checksum-verified Mongo archive was restored only into a disposable `mongod` bound to `127.0.0.1:27019`; `mongorestore` remapped `christopherbell.*` to database `test` and restored 78,239 documents with zero failures. The empty-database first candidate failed at migration 015, as the disposable DB lacked the site's migration/schema state. With restored data and default `test` profile (no collector overrides), the final candidate on port 18081 reached HTTP 200 homepage and readiness (`UP`), and logs contained no WFL or Cane's collection catch-up. Candidate PID 34120 and isolated Mongo PID 14100 were stopped; ports 18081 and 27019 are closed and production Mongo PID 5388 remains. The restored temp data directory remains because host review blocked recursive removal; no bypass was attempted. Runtime report: `docs/test-reports/2026-09-24-test-profile-collector-startup-isolation.md`.

Hosted merge-triggered checks for PR #1432 (MongoDB-only removal) and PR #1435 (auto-deploy status access-denied classification) are now all green, including Linux/macOS/Windows CI, CodeQL, and dependency review. Task 37's source change is still local, uncommitted, and awaiting required Builder report publication before opening its PR. Production deployment and final site-audit closure have not been performed.
...
## 2026-09-24 16:10 Central Daylight Time - candidate automatic-deploy health recovery

Implemented Task 38 on isolated worktree `A:\Projects\christopherbell.dev-worktrees\auto-deploy-live-health-20260924`, branch `codex/auto-deploy-live-health-20260924`, based on merged main `e166f7db9334b117f45e43e3bb7978bdcaf70ca3`. The SYSTEM poller now checks service/readiness before remote Git access, attempts a guarded recovery under the deployment lock only with exact `TARGET_ACTIVE` schema direction, starts a stopped service or restarts a running but unready service, and bounds failed recovery using separate persisted SHA/time state. The non-elevated `auto-status` projection overlays the live service/readiness state while preserving the last poller deployment result and using static sanitized messages/reasons. The guarded restart operation now starts an already stopped service rather than calling `Restart-Service` on it. Updated the Windows production runbook.

The same-SHA/stopped-service and misleading status regressions failed before implementation; recovery-under-remote-failure and state-write-failure reporting regressions were also added. Automatic-deploy (69), deploy (97), operations (102), and command (17) Pester suites passed in PowerShell 7.6.6/Pester 5.9 and Windows PowerShell 5.1.26100.9444/Pester 5.9: 285/285 in each runtime. PowerShell parser and `git diff --check` passed.

A read-only run of candidate `prod.ps1 auto-status` against the production host exited 0 and correctly reported `SERVICE_UNHEALTHY`, `deploymentStatus=BACKING_OFF`, `serviceState=STOPPED`, and `siteHealthReason=SERVICE_STOPPED`; the service remained stopped and public readiness remained HTTP 502. This candidate CLI probe does not prove the SYSTEM poller recovery or service startup. No production service, listener, configuration, ACL, deployment task, or database mutation was made. Exact launcher root cause, elevated recovery, and production acceptance remain unresolved. Builder plan and incident report were updated; implementation PR and hosted gates are next.


## 2026-09-24 16:16 - Opened guarded automatic website recovery PR

Opened PR #1437 from `codex/auto-deploy-live-health-20260924` at source commit `70593dad9d20d761b36c4e4b345c96fffb1bc129`, based on current main `e166f7db9334b117f45e43e3bb7978bdcaf70ca3`. Attached the PR to the active Codex task. All seven hosted gates were pending at 2026-09-24 21:16 UTC. The first PR create attempt correctly failed because the candidate branch had no commit; committed the reviewed five-file diff and pushed it, then opened the PR. Production remains stopped and public readiness remains HTTP 502; no production mutation occurred. Builder plan/report now identify the PR and pending hosted gates. Next: wait for CI, resolve failures, merge only after all required gates pass, then inspect auto-deploy recovery status and verify the live service and public endpoints. Do not claim recovery until those checks pass.


## 2026-09-24 16:29 - Merged recovery fix; guarded production recovery failed

PR #1437 passed every required hosted gate (Windows, macOS, Ubuntu, Java/Kotlin, JavaScript/TypeScript, Actions, CodeQL, dependency review) and squash-merged at 2026-09-24 21:22 UTC as `3b11df8a10846adf5a686f1917254ec7583e3e2f`. The SYSTEM automatic deployer refreshed tools to the merge SHA and attempted guarded website recovery. `auto-status` then reported `SERVICE_UNHEALTHY`, deployment `DEPLOYMENT_FAILED`, failure category `CANDIDATE_STARTUP`, active release `4b552a63a08c9333bdaa0d7827b23eb811920f37`, and tool source `3b11df8a10846adf5a686f1917254ec7583e3e2f`. The service remained stopped and public readiness returned HTTP 502. A read-only System/Application event query found no matching failure details; protected status intentionally omits exception content.

Attempted to open the already-authorized elevated `prod.cmd restart` path. `Start-Process -Verb RunAs` remained waiting and then returned `The operation was canceled by the user` before the elevated PowerShell process started. The restart command did not execute. No elevated production mutation occurred. The current blocking step is a guarded restart from Administrator PowerShell so the precise startup/precondition diagnostic can be read and the service can be recovered. Do not claim the site is fixed until readiness and public routes pass.


## 2026-09-24 16:35 - Found deployer liveness deadlock after recovery failure

After PR #1437 merged, current `auto-status` on the production host showed `toolSourceSha=3b11df8a10846adf5a686f1917254ec7583e3e2f` and repeated `CANDIDATE_STARTUP` backoff while the service remained stopped. Source review of the merged `Invoke-AutoDeployOnce` found a liveness defect: recovery exceptions and active-recovery backoff return before `Get-RemoteMainSha`, so the poller cannot discover or deploy a newer release while the old service is unhealthy. Added Task 39 to the existing site bug audit plan before implementation. Proposed contract preserves lock/schema guards and same-SHA recovery backoff but continues to remote revision detection, allowing a newer SHA only through normal guarded deployment and candidate checks. Added no source code yet.

Current host checks at 2026-09-24 21:35 UTC: service `Stopped`, `sc queryex` Win32 exit code 1067, public readiness HTTP 502, active SHA `4b552a63a08c9333bdaa0d7827b23eb811920f37`, tools source `3b11df8a10846adf5a686f1917254ec7583e3e2f`. Non-elevated `schtasks /run` returned access denied. The prior UAC attempt was canceled before the administrator process started. The deployed recovery flow's automatic retry remains delayed by its configured backoff.


## 2026-09-24 16:50 - Fixed auto-deploy liveness deadlock after recovery failure

Implemented the Task 39 liveness fix on branch `codex/deploy-unhealthy-release-forward-20260924`, based on merged main `3b11df8a10846adf5a686f1917254ec7583e3e2f`. Root cause: `Invoke-AutoDeployOnce` propagated an active-release recovery failure or returned during recovery backoff before checking remote `main`; this could strand the poller on an unhealthy release and prevent deploying a later fix. Added a regression that failed before the change with recovery stopping remote lookup. The code now continues remote lookup after recovery failure/backoff, still leaves same-SHA status unhealthy/backed off, and sends only a newer SHA through normal deployment guards/candidate verification. Recovery state is cleared after successful new deployment. Documentation updated.

Additional regressions cover deployment while recovery is rate-limited and remote-check failure retaining protected recovery retry state. All four Windows suites passed 288/288 under PowerShell 7.6.6/Pester 5.9 and 288/288 under Windows PowerShell 5.1/Pester 5.9. Parser checks and `git diff --check` passed. The Pester 5.9 module was explicitly imported for Windows PowerShell 5.1; system default Pester 3.4 is incompatible with this suite and its initial run was discarded. At 2026-09-24 21:35 UTC the host still reported service stopped/SCM 1067 and public readiness HTTP 502; this branch has not merged or deployed. Task 39 is recorded in the Builder plan and runtime report. Next: publish Builder evidence, push/open PR, await required gates, merge, then verify whether the deployer can deliver a newer release and restore production.
