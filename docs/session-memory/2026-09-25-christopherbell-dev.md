...
## 2026-09-25 16:02 Central Daylight Time - Safe deployment diagnostics and production success

Continued the approved deployer KISS/robustness work without elevated access. Four attempts to deploy `7639e3f1b552013a72cb59f0e32807fc6356daf3` timed out in smoke checks; the guarded poller restored `4b552a63a08c9333bdaa0d7827b23eb811920f37` each time, and non-elevated status confirmed the service/site healthy after rollback. The persisted failure detail redacted the endpoint, so the exact earlier route could not be identified.

Added safe route labels for only the fixed smoke endpoints on approved public hosts or loopback smoke ports. The first regression exposed a false route label from a trailing-dot path; independent review also found URI dot-segment and PowerShell case-insensitive switch aliases. The final implementation checks exact raw paths before URI normalization and uses case-sensitive matching. Credentials, queries, fragments, arbitrary paths, unknown hosts, invalid schemes/ports remain redacted. AutoDeploy and Command suites passed 91/91 under both PowerShell 7.6.6 and Windows PowerShell 5.1 with Pester 5.9.0; parser and diff checks passed. Independent review cleared the final patch. PR #1444 merged as `375f527910d2beae8db6ff36dc1236656a31ff9e`; CI Build, CodeQL and Dependency Review passed.
