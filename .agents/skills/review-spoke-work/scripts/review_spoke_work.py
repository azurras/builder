#!/usr/bin/env python3
"""Save a spoke work review record."""
...
def slugify(value: str) -> str:
    ...
    return re.sub(r"-{2,}", "-", value).strip("-")[:80] or "spoke-review"


def main() -> int:
    parser = argparse.ArgumentParser(description="Save docs/spoke-reviews/YYYY-MM-DD-title.md.")
    ...
    if not body:
        print("Review body is required on stdin", file=sys.stderr)
        return 2
    out_dir = Path(args.root).expanduser().resolve() / "docs" / "spoke-reviews"
