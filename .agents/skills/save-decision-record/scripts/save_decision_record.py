...
def slugify(value: str) -> str:
    ...
    return re.sub(r"-{2,}", "-", value).strip("-")[:80] or "decision"


def main() -> int:
    parser = argparse.ArgumentParser(description="Save docs/decisions/YYYY-MM-DD-title.md.")
    ...
    if not body:
        print("Decision body is required on stdin", file=sys.stderr)
        return 2
    out_dir = Path(args.root).expanduser().resolve() / "docs" / "decisions"
