#!/usr/bin/env python3
"""Save a hub work closure record."""
...
def slugify(value: str) -> str:
    ...
    return re.sub(r"-{2,}", "-", value).strip("-")[:80] or "work-closure"


def main() -> int:
    parser = argparse.ArgumentParser(description="Save docs/work-closures/YYYY-MM-DD-title.md.")
    ...
    if not body:
        print("Closure body is required on stdin", file=sys.stderr)
        return 2
    out_dir = Path(args.root).expanduser().resolve() / "docs" / "work-closures"
