...
def slugify(value: str) -> str:
    ...
    return re.sub(r"-{2,}", "-", value).strip("-")[:80] or "spoke-update"


def main() -> int:
    parser = argparse.ArgumentParser(description="Save docs/spoke-updates/YYYY-MM-DD-title.md.")
    ...
    if not body:
        print("Spoke update body is required on stdin", file=sys.stderr)
        return 2
    out_dir = Path(args.root).expanduser().resolve() / "docs" / "spoke-updates"
