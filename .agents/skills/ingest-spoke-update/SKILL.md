...
description: Record a returned update from an agent working in a spoke repository back into the Azurras hub. Use when a spoke task reports progress, commits, pull requests, validation results, blockers, or completion status.
---
...
Preserve spoke repo results in Azurras so the hub remains the source of truth for cross-repo work.
...
4. Update session memory and use `commit-push-azurras-main`.
