...
Pass complete Markdown on stdin to `python .agents/skills/plan-builder-work/scripts/save_decision_record.py --root . --title 'Decision title'`. It saves under `docs/decisions/` and refuses duplicates without `--overwrite`. Finish the current artifact phase with the [phase finalizer](../../maintain-builder-hub/references/phase-finalization.md); no extra memory cycle is required.
